        ...
        <div class="card-header">
            <h4>
                ...
                @if (Auth()->user()->user_role != 0)
                    <a href="view-chart/dist-past-r/{{ $destination }}/{{ $area['area'] }}/{{ $area['district'] }}/{{ $area['local'] }}" class="btn btn-info float-end" style="margin-left: 10px;">Report</a>
                ...
                <tbody>
                    @foreach ($data as $key => $data)
                        <tr id="sid{{ $data->district_pastor_id }}">
                            ...
                            @if (Auth()->user()->user_role != 0)
                                <td>
                                    <a href="edit-form/{{ $data->district_pastor_id }}/dist_past_r" class="btn btn-success btn-sm" style="text-decoration: none" title="Delete">Edit</a>
                                    @if (Auth()->user()->user_role == 1)
                                        <a href="javascript:void(0)"  onclick="deleteRecord({{ $data->district_pastor_id }}, 'dist_past_r')" class="btn btn-danger btn-sm" style="text-decoration: none" title="Delete">Delete</a> 
                                    @endif
                                </td>
                            @endif
                        </tr>
