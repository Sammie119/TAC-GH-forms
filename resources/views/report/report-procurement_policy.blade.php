                ...
                <tbody>
                    @foreach ($data as $key => $data)
                        <tr id="sid{{ $data->area_head_id }}">
                            ...
                            <td>{{ $data->district }}</td>
                            <td>{{ $data->local }}</td>
                            <td>{{ $data->fin_eva_b_moniter }}</td>
                            <td>{{ $data->user_name }}</td>
                            @if (Auth()->user()->user_role != 0)
                                <td>
                                    <a href="edit-form/{{ $data->procurement_id }}/pro_policy" class="btn btn-success btn-sm" style="text-decoration: none" title="Delete">Edit</a>
                                    @if (Auth()->user()->user_role == 1)
                                        <a href="javascript:void(0)"  onclick="deleteRecord({{ $data->procurement_id }}, 'pro_policy')" class="btn btn-danger btn-sm" style="text-decoration: none" title="Delete">Delete</a>
                                    @endif
                                </td>
                            @endif
                        </tr>
